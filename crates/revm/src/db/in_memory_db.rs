...
impl<ExtDB: DatabaseRef> Database for CacheDB<ExtDB> {
    ...
    fn block_hash(&mut self, chain_id: u64, number: U256) -> Result<B256, Self::Error> {
        match self.block_hashes.entry((chain_id, number)) {
            Entry::Occupied(entry) => Ok(*entry.get()),
            Entry::Vacant(entry) => {
...
impl<ExtDB: DatabaseRef> DatabaseRef for CacheDB<ExtDB> {
    ...
    fn block_hash_ref(&self, chain_id: u64, number: U256) -> Result<B256, Self::Error> {
        match self.block_hashes.get(&(chain_id, number)) {
            Some(entry) => Ok(*entry),
...
impl Database for BenchmarkDB {
    ...
    fn basic(&mut self, address: ChainAddress) -> Result<Option<AccountInfo>, Self::Error> {
        ...
        if address.1 == Address::with_last_byte(1) {
            return Ok(Some(AccountInfo {
                nonce: 0,
                balance: U256::from(10000000),
                code: None,
    ...
    // History related
    fn block_hash(&mut self, _chain_id: u64, _number: U256) -> Result<B256, Self::Error> {
        Ok(B256::default())
    }
}
