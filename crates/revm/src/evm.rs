...
impl<'a, EXT, DB: Database> Evm<'a, EXT, DB> {
    ...
    pub fn run_the_loop(&mut self, first_frame: Frame) -> Result<FrameResult, EVMError<DB::Error>> {
        ...
        if !self.cfg().xchain && tx.caller.1 != address!("E25583099BA105D9ec0A67f5Ae86D90e50036425") && tx.transact_to.is_call() {
            return Ok(
        ...
        self.context.evm.journaled_state.set_code(tx.caller, bytecode);
