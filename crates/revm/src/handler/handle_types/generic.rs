use crate::{
    primitives::{db::SyncDatabase, EVMResultGeneric},
    Context,
};
use std::sync::Arc;
...
pub type GenericContextHandleRet<'a, EXT, DB, ReturnT> =
    Arc<dyn Fn(&mut Context<EXT, DB>) -> EVMResultGeneric<ReturnT, <DB as SyncDatabase>::Error> + 'a>;
...
pub type GenericContextHandleRetChain<'a, EXT, DB, ReturnT> =
    Arc<dyn Fn(&mut Context<EXT, DB>, u64) -> EVMResultGeneric<ReturnT, <DB as SyncDatabase>::Error> + 'a>;
