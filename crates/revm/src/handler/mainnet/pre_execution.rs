...
use crate::{
    ...
    primitives::{
        ...
        TransactTo, TxKind, BLOCKHASH_STORAGE_ADDRESS, KECCAK_EMPTY, U256,
    },
...
pub fn apply_eip7702_auth_list<SPEC: Spec, EXT, DB: Database>(
    ...
    for authorization in authorization_list.recovered_iter() {
        ...
        if chain_id != 0 && !context.evm.inner.env.tx.chain_ids.clone().unwrap_or_default().contains(&chain_id) {
            continue;
        }
        ...
        let mut authority_acc = context
            ...
            .inner
            .journaled_state
            .load_code(ChainAddress(chain_id, authority), &mut context.evm.inner.db)?;
