...
use revm::{
    ...
    primitives::{
        ...
        ExecutionResult, Output, TransactTo, B256, KECCAK_EMPTY, U256,
    },
    Evm,
};
...
fn main() {
    ...
    db.insert_account_storage(ChainAddress(L2, deployment), U256::from(0), U256::from(2))
        .expect("failed to set storage");

    insert_account_info(&mut db, ChainAddress(L1, deployment), Owner::BYTECODE.clone());
    db.insert_account_storage(ChainAddress(L1, deployment), U256::from(0), U256::from(1))
        .expect("failed to set storage");
    ...
    let mut evm = Evm::builder()
        .modify_tx_env(|tx| {
            tx.caller = l1_user;
            tx.transact_to = TransactTo::Call(ChainAddress(L1, deployment));
    ...
    assert_eq!(
        U256::from_str(&result.output().unwrap().to_string()),
        U256::from_str("1")
    );
    drop(evm);
    ...
    let mut evm = Evm::builder()
        .modify_tx_env(|tx| {
            tx.caller = l2_user;
            tx.transact_to = TransactTo::Call(ChainAddress(L2, deployment));
